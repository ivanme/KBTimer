...
#define WM_SET_BACKLIGHT (WM_USER + 1026)

void setBrightness(HWND kbHWND, int brightness)
{
	//For some reason, if backlight is off, it must first be set to 1 before any higher value.
	//(eg. When current value is 0, setting it to 2 will have no effect on the keyboard.)
	if (brightness > 1)
	{
		PostMessage(kbHWND, WM_SET_BACKLIGHT, 1, 0);
	}

	PostMessage(kbHWND, WM_SET_BACKLIGHT, brightness, 0);
}
